...
For troubleshooting, see link:https://github.com/foosel/OctoPrint/wiki/FAQ#i-cant-get-octoprint-to-connect-to-my-printer
[Octoprint’s documentation] as well.
...
You can only upload STL or G-Code files. Typically STL files have the extension .stl and G-code files 
have either the .gco or .gcode extension. 
Click one of the upload buttons found on the left of the screen - see image below. 
Upload transfers the file to the Pi, while Upload to SD transfers the file to the printer’s SD card. 
